...
/// Heartbeat info is requested from multiple processes when the storage directory already exists
/// (i.e. created by a previous app launch).
- (void)testHeartbeatDateForTagFromMultipleProcesses {
  NSMutableArray<XCTestExpectation *> *expectations = [NSMutableArray array];

  NSArray<NSNumber *> *priorities = @[
    @(DISPATCH_QUEUE_PRIORITY_DEFAULT), @(DISPATCH_QUEUE_PRIORITY_HIGH),
    @(DISPATCH_QUEUE_PRIORITY_BACKGROUND)
  ];
  NSUInteger dispatchCount = 5;
  // Dispatch heartbeat APIs `dispatchCount` times on concurrent queues with random priorities.
  for (NSUInteger i = 0; i <= dispatchCount; i++) {
    NSInteger randomIndex = arc4random_uniform((int)priorities.count);
    NSNumber *priority = priorities[randomIndex];

    NSString *expectationDescription =
        [NSString stringWithFormat:@"Process on queue %@", priority.stringValue];
    XCTestExpectation *expectation = [self expectationWithDescription:expectationDescription];
    [expectations addObject:expectation];

    NSDate *storedDate = [NSDate date];
    NSString *storedTag = [NSString stringWithFormat:@"fire-tag-%@", @(i)];
    // The below method call will create the heartbeat storage directory as a side effect.
    [self.storage setHearbeatDate:storedDate forTag:storedTag];

    dispatch_queue_t queue = dispatch_get_global_queue(priority.intValue, 0);
...
- (void)testHeartbeatDateForTagFromMultipleProcessesAfterManuallyCreatingDirectory {
  // 1. Manually create the heartbeat directory.
  NSURL *heartbeatStorageDirectoryURL = [self pathURLForDirectory:kGULHeartbeatStorageDirectory];
  NSError *error;
  BOOL directoryCreated = [self explicitlyCreateDirectoryForURL:heartbeatStorageDirectoryURL
  ...
  for (NSUInteger i = 0; i <= dispatchCount; i++) {
    ...
    dispatch_async(queue, ^{
      ...
      NSString *storedTag = [NSString stringWithFormat:@"fire-tag-%@", @(i)];
      // The below method call will create the heartbeat storage directory as a side effect.
      BOOL success = [self.storage setHearbeatDate:storedDate forTag:storedTag];
      XCTAssert(success);
      NSDate *retrievedDate = [self.storage heartbeatDateForTag:storedTag];
      XCTAssertEqualObjects(retrievedDate, storedDate);
...
/// Heartbeat info is requested from multiple processes when the storage directory does not yet
/// exist (e.g. initial app launch) and must
/// be lazily created within one of the processes.
// Create directory manually, possibily an ineternal serial queue
- (void)testSynchronouslyCoordinateOnHeartbeatStorageDirectoryCreation {
  NSMutableArray<XCTestExpectation *> *expectations = [NSMutableArray array];
